...
def parse_features(pages):
    ...
    if "hair density is poor" in full_text:
        data["hair_density"] = "poor"
    elif "hair density is medium" in full_text:
        data["hair_density"] = "medium"
    elif "hair density is good" in full_text:
        data["hair_density"] = "good"
    else:
        data["hair_density"] = None
    
    data["alopecia_patch"] = "patch of alopecia" in full_text and "not seen" not in full_text.split("patch of alopecia")[0][-20:]
    
    if "dandruff seen in large" in full_text:
        data["dandruff_severity"] = "large"
    elif "dandruff seen in moderate" in full_text:
        data["dandruff_severity"] = "moderate"
    elif "dandruff seen in low" in full_text or "dandruff seen in small" in full_text:
        data["dandruff_severity"] = "low"
    else:
        data["dandruff_severity"] = None
    
    graying = re.search(r'graying of hair seen\s*(\d+)-?(\d+)?', full_text)
    if graying:
        if graying.group(2):
            data["graying_percentage"] = f"{graying.group(1)}-{graying.group(2)}%"
        else:
            data["graying_percentage"] = f"{graying.group(1)}%"
    else:
        data["graying_percentage"] = None
    
    follicle = re.search(r'no\.\s*of hair per follicle is\s*([\d-]+)', full_text)
    ...
    medulla = re.search(r'your medulla pattern\s*:\s*(\w+)', full_text)
    data["medulla_pattern"] = medulla.group(1).strip() if medulla else None
    
    data["floating_test"] = "hair floating test is positive" in full_text
    
    if data["floating_test"]:
        data["porosity"] = "High"
    else:
        data["porosity"] = "Low"
    ...
    for pattern, key in saptadhatu_patterns:
        match = re.search(pattern, page9_text, re.IGNORECASE)
    ...
    data["kesh_saamata_parikshan"] = analysis_match.group(1).strip() if analysis_match else ""

    page11_text = pages.get(11, "")
    prakruti_match = re.search(r'note\s*[:\-]?\s*(.*?)(?:\n|$)', page11_text, re.IGNORECASE)
    data["kesh_prakruti_parikshan"] = prakruti_match.group(1).strip() if prakruti_match else ""
...
def analyze_bars(img, width, page_num):
    ...
    bar_start, bar_end = width * 0.31, width * 0.94
    ...
    for i, (cX, cY) in enumerate(centers):
        if i >= len(manual_labels): 
            break
        rel_pos = (cX - bar_start) / bar_width
                ...
                for img_path in image_paths:
                    ...
                    if layout_key in cfg:
                        ...
                        if bar_img is not None:
                            results = analyze_bars(bar_img, original_width, pg_num)
